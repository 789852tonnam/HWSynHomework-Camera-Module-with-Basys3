...
// === line_buffer3 module - 3-line circular shift buffer for 3x3 Sobel ===
module line_buffer3 #(
    parameter WIDTH = 640
) (
    input  wire        clk,
    ...
    always @(posedge clk) if (pix_valid) lb[wr_idx][h_count] <= pix_in;

    always @(posedge clk) begin
        if (rst) wr_idx <= 2'd0;
        else if (pix_valid && h_count == WIDTH - 1) wr_idx <= (wr_idx == 2'd2) ? 2'd0 : wr_idx + 2'd1;
    end
    ...
    always @(posedge clk) begin
        top_l <= lb[rd_top_idx][col_l]; top_c <= lb[rd_top_idx][col_c]; top_r <= lb[rd_top_idx][col_r];
        mid_l <= lb[rd_mid_idx][col_l]; mid_c <= lb[rd_mid_idx][col_c]; mid_r <= lb[rd_mid_idx][col_r];
        bot_l <= lb[rd_bot_idx][col_l]; bot_c <= lb[rd_bot_idx][col_c]; bot_r <= lb[rd_bot_idx][col_r];
    end

endmodule

// === filter_edge module - 3x3 Sobel edge detector ===
module filter_edge (
    input  wire [2:0]  top_l, top_c, top_r,
    input  wire [2:0]  mid_l, mid_c, mid_r,
    input  wire [2:0]  bot_l, bot_c, bot_r,
    input  wire [3:0]  threshold,
    input  wire        h_edge, v_edge,
    output wire [11:0] out_rgb444
);

    wire [2:0] y00=top_l, y01=top_c, y02=top_r, y10=mid_l, y12=mid_r, y20=bot_l, y21=bot_c, y22=bot_r;
    ...
    assign out_rgb444 = (h_edge || v_edge) ? 12'h000 : (mag > {3'b000, threshold}) ? 12'hFFF : 12'h000;
